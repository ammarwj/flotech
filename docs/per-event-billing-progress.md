...
- [x] **Riwayat verifikasi di `/admin/plan-orders`** — `GET admin/plan-orders/history`, tab ketiga di halaman yang sama (`PillTabs`; ketiga daftar dulu bertumpuk, jadi antrean — satu-satunya yang ada kerjaannya — terbaca seperti catatan kaki di halamannya sendiri). Buktinya bisa dibuka lagi lewat **`PaymentProofDialog` yang sama tanpa `onApprove`/`onReject`**: mencari bukti lama itu perbuatan yang sama dengan mencarinya untuk diputuskan, cuma tanpa putusan — jadi itu satu-satunya yang dihilangkan. Keduanya dipegang **satu** state (`{ order, settled }`), bukan dua, supaya dialog tidak mungkin dapat baris antrean tanpa tombol atau baris lunas dengan tombol Terima yang akan memutus perkara dua kali. Filternya `verified_at IS NOT NULL`, **kolom yang sama** yang `scopeAwaitingVerification()` haruskan null, jadi satu baris ada di tepat satu dari dua daftar dan tidak bisa hilang dari keduanya. Yang **ditolak tidak masuk riwayat**: `rejectProof()` sengaja membiarkan `verified_at` null supaya unggahan ulang kembali ke antrean, jadi ini log persetujuan, bukan log keputusan. `verified_by` (nama admin) cuma dikirim saat relasi `verifier` di-load — antrean belum punya siapa pun untuk disebut. Dibatasi 50 baris, bukan paginasi: pertanyaannya "transfer ini sudah pernah kita acc belum?", dan arsip permanennya tetap PDF kwitansi. Ujilah dengan **membandingkan** kedua daftar di dua sisi persetujuan (1/0 → 0/1); assert "riwayat tidak kosong" akan lolos walau barisnya tidak pernah keluar dari antrean dan bisa di-acc dua kali.
