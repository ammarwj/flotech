...
const fmtDate = (d: string) =>
  format(parseISO(d), "d MMM yyyy", { locale: idLocale });
      ...
      <CardContent className="space-y-3 p-4">
        ...
        <div className="space-y-2 border-t border-border pt-3">
          <SummaryRow icon={CalendarDays}>
            ...
            {days ? (
              <span className="text-muted-foreground"> · {days} hari</span>
            ) : null}
          </SummaryRow>
          <SummaryRow icon={MapPin}>
            {v.location_name?.trim() || (
              <span className="text-muted-foreground">Lokasi belum diatur</span>
            )}
          </SummaryRow>
        </div>
        <div className="space-y-2 border-t border-border pt-3">
          ...
          {categories.map((c) => {
            ...
            return (
              <div
                key={c._key}
                className="rounded-md bg-[var(--bg-soft)] px-2.5 py-2 text-sm"
              >
                <p className="font-semibold">
                  {c.name?.trim() || "Kategori tanpa nama"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {formatLabel(c.tournament_format)} ·{" "}
                  {free ? "Gratis" : rupiah(c.registration_fee ?? 0)}
  ...
  return (
    <div className="grid gap-4 rounded-lg border border-border p-3 sm:p-4">
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              onChange({
                participant_type,
                rubber_format:
                  participant_type === "team" ? cat.rubber_format : null,
              });
            }}
          >
            {modes.map((m) => (
      ...
      {modes.includes("single") &&
        (cat.participant_type ?? "team") === "team" && (
          <RubberFormatEditor
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="grid gap-2">
          ...
          <Input
            ...
            onChange={(e) =>
              onChange({
                max_teams: e.target.value ? Number(e.target.value) : undefined,
              })
            }
            placeholder={
              teamsCap !== null ? `Maks ${teamsCap}` : "Tidak dibatasi"
            }
          />
          <FieldHint>
            {teamsCap !== null
        ...
        <div className="grid gap-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <label className="flex cursor-pointer items-center gap-1.5 text-xs font-medium text-muted-foreground">
              <input
                ...
                checked={free}
                onChange={(e) =>
                  onChange({ registration_fee: e.target.checked ? 0 : 1 })
                }
              />
              Gratis
            </label>
          ...
          <div className="relative">
            ...
            <Input
              ...
              onChange={(e) =>
                onChange({
                  registration_fee: e.target.value ? Number(e.target.value) : 0,
                })
              }
              className="pl-9"
            />
          ...
          <FieldHint>
            {free ? "Gratis untuk peserta." : rupiah(cat.registration_fee ?? 0)}
          </FieldHint>
        </div>
      </div>
    </div>
...
export function EventForm({
  ...
  const [v, setV] = useState<EventInput>({
    ...
    payment_method:
      initial?.payment_method ?? (gatewayInPlan ? "gateway" : "manual"),
  ...
  const errorFor = (k: keyof EventInput): string | undefined =>
    (k in clientErrors
      ? clientErrors[k as string]
      : fieldErrors?.[k as string]) || undefined;
  ...
  const engineOf = (key?: string) =>
    tournament_formats.find((f) => f.key === key)?.meta?.engine as
      | string
      | undefined;
  ...
  const addCat = () =>
    setCategories((cs) => [
      ...cs,
      newCategory(tournament_formats[0]?.key ?? ""),
    ]);
  const removeCat = (key: string) =>
    setCategories((cs) =>
      cs.length > 1 ? cs.filter((c) => c._key !== key) : cs,
    );
  ...
  const bannerShown =
    bannerPreview ??
    (v.banner_url && /^https?:\/\//.test(v.banner_url) ? v.banner_url : null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setAttestError(
      attestBad
        ? "Kamu harus menyetujui pernyataan ini untuk melanjutkan."
        : undefined,
    );
            ...
            <CardContent className="grid gap-4">
              <div className="grid gap-2">
                ...
                {errorFor("name") ? (
                  ...
                  <FieldHint>
                    Tampil sebagai judul di halaman publik event.
                  </FieldHint>
                )}
              </div>
              ...
              <div className="grid gap-2 sm:max-w-[50%] sm:pr-2">
                ...
                <Select
                  ...
                  onChange={(e) =>
                    set(
                      "sport_type",
                      e.target.value as EventInput["sport_type"],
                    )
                  }
                >
                  {sports.map((s) => (
                ...
                <FieldHint>
                  Semua kategori memakai cabang olahraga yang sama.
                </FieldHint>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="grid gap-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <InfoHint label="Penjelasan zona waktu">
                      Zona waktu lokasi pertandingan. Jam yang kamu isi di
                      jadwal berarti jam setempat, dan penonton di zona mana
                      pun akan melihat jam yang sama.
                    </InfoHint>
                  </div>
                  <Select
                    id="timezone"
              ...
              <div className="grid gap-2 sm:max-w-[50%] sm:pr-2">
                ...
                <Select
                  id="payment_method"
                  value={
                    gatewayDown ? "manual" : (v.payment_method ?? "gateway")
                  }
                  disabled={gatewayDown}
                  onChange={(e) =>
                    set(
                      "payment_method",
                      e.target.value as EventInput["payment_method"],
                    )
                  }
                ...
                {errorFor("payment_method") ? (
                  ...
                  <FieldHint>
                    Payment gateway platform sedang dimatikan, jadi semua event
                    memakai transfer manual. Pilihanmu tersimpan dan berlaku
                    lagi begitu gateway dinyalakan.
                  </FieldHint>
                ) : !gatewayInPlan ? (
                  <FieldHint>
                    Paket event ini tidak termasuk pembayaran online. Naikkan
                    paket kalau ingin memakai Midtrans.
                  </FieldHint>
                ) : noBank && v.payment_method === "manual" ? (
                  <FieldHint>
                    Transfer manual butuh rekening tujuan — isi rekening
                    penarikan di{" "}
                    <Link
                  ...
                  <FieldHint>
                    Online kena fee platform. Manual tanpa potongan, tapi kamu
                    yang verifikasi tiap bukti transfer.
                  </FieldHint>
                )}
              </div>
            </CardContent>
            ...
            <CardContent className="grid gap-4">
              {categories.map((c, i) => (
                <CategoryEditor
                  ...
                  isHybrid={
                    engineOf(c.tournament_format || fallbackFormat) === "hybrid"
                  }
                  isLeague={
              ...
              <Button
                ...
                className="gap-2"
                disabled={
                  categoryCap !== null && categories.length >= categoryCap
                }
                onClick={addCat}
              >
              ...
              <CardContent className="grid gap-4">
                ...
                <div className="grid items-end gap-4 sm:grid-cols-3">
                  <div className="grid gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <Label
                        htmlFor="yellow-threshold"
                        className="font-semibold"
                      >
                        {sportCards.yellow?.label ?? "Kartu kuning"} untuk 1
                        larangan
                      </Label>
                      <InfoHint label="Penjelasan akumulasi kartu kuning">
                        Berapa {yellowWord} yang{" "}
                        <b>menumpuk sepanjang turnamen</b> sebelum pemain kena
                        larangan. Terkumpul di laga mana saja — 1 di laga 2, 1
                        di laga 5, 1 di laga 9 sudah cukup. Begitu larangan
                        terbit, hitungannya kembali ke nol.
                      </InfoHint>
                    </div>
                    <Input
                      id="yellow-threshold"
                      ...
                      placeholder={
                        sportDiscipline.yellow_threshold?.toString() ?? "3"
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <InfoHint label="Penjelasan lama larangan akumulasi">
                        Berapa pertandingan pemain absen setelah {yellowWord}
                        -nya menumpuk sampai batas di sebelah kiri. Isi <b>
                          0
                        </b>{" "}
                        kalau akumulasi di turnamen ini tidak berbuah larangan
                        sama sekali.
                      </InfoHint>
                    </div>
                    <Input
                      ...
                      onChange={(e) =>
                        setDiscipline((d) => ({
                          ...d,
                          yellow_ban_matches: e.target.value,
                        }))
                      }
                      placeholder={
                        sportDiscipline.yellow_ban_matches?.toString() ?? "1"
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <InfoHint
                        label="Penjelasan lama larangan kartu merah"
                        align="end"
                      >
                        Berapa pertandingan pemain absen setelah menerima{" "}
                        {redWord}. Berlaku langsung, tanpa menunggu kartu lain.
                        Kalau di satu laga tercatat {redWord} <b>dan</b>{" "}
                        {yellowWord} sekaligus, sistem membacanya sebagai satu
                        kejadian — larangannya tidak dobel.
                      </InfoHint>
                    </div>
                    <Input
                      id="red-ban"
                      ...
                      onChange={(e) =>
                        setDiscipline((d) => ({
                          ...d,
                          red_ban_matches: e.target.value,
                        }))
                      }
                      placeholder={
                        sportDiscipline.red_ban_matches?.toString() ?? "1"
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                    <div className="flex items-center gap-1.5">
                      <Label
                        htmlFor="yellow-expulsion"
                        className="font-semibold"
                      >
                        {sportCards.yellow?.label ?? "Kartu kuning"} 1 laga =
                        dikeluarkan
                      </Label>
                      <InfoHint label="Penjelasan dikeluarkan karena kartu kuning">
                        Berapa {yellowWord} <b>di dalam satu pertandingan</b>{" "}
                        yang dihitung sebagai pengusiran — bukan penumpukan
                        lintas laga seperti kolom paling kiri. {yellowWord} yang
                        berujung pengusiran <b>tidak ikut dihitung lagi</b> ke
                        akumulasi, supaya satu kejadian tidak dihukum dua kali.
                        Isi <b>0</b> untuk mematikan aturan ini.
                      </InfoHint>
                    </div>
                    <Input
                      id="yellow-expulsion"
                      ...
                      onChange={(e) =>
                        setDiscipline((d) => ({
                          ...d,
                          yellows_per_expulsion: e.target.value,
                        }))
                      }
                      placeholder={
                        sportDiscipline.yellows_per_expulsion?.toString() ?? "2"
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <InfoHint label="Penjelasan lama larangan dikeluarkan">
                        Berapa pertandingan pemain absen setelah dikeluarkan
                        karena {yellowWord} di laga yang sama. Terpisah dari
                        larangan akumulasi — keduanya bisa berjalan berurutan
                        kalau memang dua kejadian berbeda.
                      </InfoHint>
                    </div>
                    <Input
                      id="expulsion-ban"
                      ...
                      onChange={(e) =>
                        setDiscipline((d) => ({
                          ...d,
                          expulsion_ban_matches: e.target.value,
                        }))
                      }
                      placeholder={
                        sportDiscipline.expulsion_ban_matches?.toString() ?? "1"
                      }
                    />
                  </div>
                </div>
                ...
                <FieldHint>
                  ...
                  placeholder itulah yang sedang berlaku. Larangan dianggap
                  dijalani di pertandingan resmi tim berikutnya, dan sistem
                  hanya memperingatkan: keputusan menurunkan pemain tetap di
                  tangan panitia.
                </FieldHint>
                ...
                <label className="flex items-start gap-3">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setDiscipline((d) => ({
                        ...d,
                        reset_yellow_on_knockout: checked,
                      }))
                    }
                  />
                  <span className="grid gap-1">
                    ...
                    <FieldHint>
                      Hanya berlaku untuk kategori Grup + Knockout — cuma format
                      itu yang menandai laganya sebagai babak gugur. Larangan
                      yang sudah terbit tetap harus dijalani.
                    </FieldHint>
                  </span>
                </label>
              </CardContent>
          ...
          <Card>
            ...
            <CardContent className="grid gap-4">
              ...
              <div className="grid gap-2">
                ...
                <FieldHint>
                  Dipakai sebagai pilihan lapangan saat menjadwalkan
                  pertandingan. Kosongkan jika tidak perlu.
                </FieldHint>
              </div>
            </CardContent>
          </Card>
                ...
                {bannerShown ? (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">
                      {bannerUploading
                        ? "Mengunggah…"
                        : "Unggah gambar turnamen"}
                    </span>
                    <span className="text-xs">
                      PNG / JPG, maks 5 MB · rasio 4:5 disarankan
                    </span>
                  </button>
                )}
              </div>
      ...
      <div className="rounded-xl border border-border p-4">
        <label className="flex cursor-pointer items-start gap-2.5 text-sm">
          ...
          <span className="leading-relaxed text-muted-foreground">
            Saya menyatakan event ini{" "}
            <strong className="text-foreground">
              tidak mengandung unsur perjudian
            </strong>{" "}
            dalam bentuk apa pun, termasuk hadiah yang bersumber dari akumulasi
            biaya pendaftaran peserta, dan saya menyetujui{" "}
            <Link
              href="/ketentuan"
              target="_blank"
